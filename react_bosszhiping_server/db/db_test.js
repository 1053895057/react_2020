...
function testFind() {
    ...
    UserModel.find({ _id: '5ff193dac649541854c451b3' }, function (error, users) {
        console.log('find()', error, users)
    })
    // 查询一个: 得到是匹配的文档对象, 如果没有匹配的就是null
    UserModel.findOne({ _id: '5ff191bc2926633170d1517f' }, function (error, user) {
...
// 3.3. 通过Model的findByIdAndUpdate()更新某个数据
function testUpdate() {
    UserModel.findByIdAndUpdate({ _id: '5ff191bc2926633170d1517f' },
...
// 3.4. 通过Model的remove()删除匹配的数据
function testDelete() {
    UserModel.remove({ _id: '5ff191bc2926633170d1517f' }, function (error, doc) {
